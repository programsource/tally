...
public class TallyDetailAdapter extends StickyHeaderGridAdapter {
    ...
    public void onBindHeaderViewHolder(HeaderViewHolder viewHolder, int section) {
        ...
        float out = 0;
        float in = 0;
        for (TallyDetailBean.DaylistBean.ListBean bean : mDatas.get(section).getList()) {
            float f = Float.parseFloat(bean.getAffect_money());
            //支出
            if (f <= 0) {
                out += f;
            } else {
        ...
        holder.header_money.setText("支出: "+out+" 收入: "+in);
    }
